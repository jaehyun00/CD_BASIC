...
# 오늘은 떡볶이 떡을 만드는 날이다.
# 동빈이네 떡볶이 떡은 재밌게도 떢볶이 떡의 길이가 일정하지 않다.
...
rc_list = list(map(int, sys.stdin.readline().split()))
...
while True:
    mid_index = (0 + (len(rc_list) - 1)) // 2

    mid_height = rc_list[mid_index]

    sum_target_list = rc_list[mid_index:]

    sum_height = sum([s - mid_height for s in sum_target_list])

    if sum_height > m:
        rc_list = rc_list[mid_index:]
    else:
        cut_height = mid_index
        break

print(rc_list[cut_height])
...
# -------------------------------------------------------------------
# 문제의 핀트를 좀 잘못잡았음
# 생각이 너무 많고 단순하게 해결하려는 사고가 필요
# 이진 탐색의 원리가 중요한거지 이진 탐색의 구현이 중요하지 않은듯